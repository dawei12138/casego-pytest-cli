...
from pytest_auto_api2.utils.other_tools.models import TestMetrics
    ...
    def get_failed_case(self) -> List:
        ...
        for i in self.get_testcases():
            if i['status'] == 'failed' or i['status'] == 'broken':
                case_path = i.get('fullName') or i.get('testCaseName') or i.get('name', '')
                error_case.append((i.get('name', ''), case_path))
        return error_case
