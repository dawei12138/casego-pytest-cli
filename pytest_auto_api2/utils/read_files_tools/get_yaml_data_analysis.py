...
from pytest_auto_api2.utils.other_tools.models import RequestType, Method, TestCaseEnum
import os
    ...
    def case_process(self, case_id_switch: Union[None, bool] = None):
        ...
        for key, values in data.items():
            ...
            if key != 'case_common':
                ...
                model = TestCase(**case_date)
                model_data = self._model_to_dict(model)
                if case_id_switch is True:
                    case_list.append({key: model_data})
                else:
                    case_list.append(model_data)
